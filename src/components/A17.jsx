import React, { useRef } from "react";
import { chipImg, frameVideo, frameImg } from "../utils"; // Assuming video is in utils
import { useGSAP } from "@gsap/react";
import gsap from "gsap/all";
import { animatewithgsap } from "../utils/animations";

const A17 = () => {
  const videoRef = useRef(null);

  useGSAP(() => {
    gsap.from("#chip", {
      scrollTrigger: {
        trigger: "#chip",
        start: "20% bottom",
        toggleActions: 'play reverse play reverse',
      },
      opacity: 0,
      scale: 2,
      duration: 2,
      ease: "power2.inOut",
    });

    animatewithgsap('.g_fadeIn', {
      opacity: 1,
      y: 0,
      duration: 1,
      ease: 'power2.inOut'
    });
  }, []);

  // Once video ends it is replayed again
  const handleVideoEnd = () => {
    if (videoRef.current) {
      videoRef.current.currentTime = 0; // Reset video to the beginning
      videoRef.current.play(); // Play the video again
    }
  };

  return (
    <section className="common-padding">
      <div className="screen-max-width">
        {/* Chip Section */}
        <div id="chip" className="flex-center w-full my-20">
          <img src={chipImg} alt="chip" width={100} height={100} />
        </div>

        <div className="flex flex-col items-center">
          <h2 className="hiw-title">
            A17 Pro Chip <br />
            A Monster Win for Gaming.
          </h2>
          <p className="hiw-subtitle">
            It's here, the biggest re-design in the history of Apple GPUs.
          </p>
        </div>

        {/* Video with Frame Section */}
        <div className="mt-10 md:mt-20 mb-14">
          <div className="relative h-full flex-center z-10">
            {/* Frame and Video container */}
            <div className="frame-video">
              {/* Frame Image */}
              <img src={frameImg} alt="frame" className="bg-transparent relative z-10" />

              {/* Video inside the frame */}
              <video
                className="hiw-video" playsInline preload="none" muted autoPlay
                ref={videoRef} onEnded={handleVideoEnd} // Add the onEnded handler
              >
                <source src={frameVideo} type="video/mp4" />
              </video>
            </div>
          </div>

          <p className="text-gray font-semibold text-center mt-3">Honkai: Star Rail</p>
        </div>

        {/* Text Below the Video */}
        <div className="hiw-text-container">
          <div className="flex flex-1 flex-center justify-center flex-col">
            <p className="hiw-text g_fadeIn">
              A17 Pro is an entirely new class of iPhone chip that delivers our{" "}
              <span className="text-white">best graphic performance by far!!</span>.
            </p>

            <p className="hiw-text g_fadeIn">
              Mobile{" "}
              <span className="text-white">games will look & feel so immersive</span>, with incredibly detailed environments and characters.
            </p>
          </div>

          <div className="flex-1 flex justify-center flex-col g_fadeIn">
            <p className="hiw-text">New</p>
            <p className="hiw-bigtext">Pro-class GPU</p>
            <p className="hiw-text">with 6 cores</p>
          </div>
        </div>
      </div>
    </section>
  );
};

export default A17;
